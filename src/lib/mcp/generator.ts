...
import { generateToolWithLogic } from "./tool-logic-generator";
import { generateResources, generateResourcesFile } from "./resource-generator";
...
export class McpGenerator {
  ...
  private generateFiles(
    ...
    for (const skill of config.skills) {
      files.push({
        ...
        content: generateToolWithLogic(skill, config),
        language: "typescript",
        description: `Herramienta: ${skill.name} (${config.accessStrategy || "code_input"} strategy)`,
      });
    }

    // Resources file (auto-generated context for AI)
    const resources = generateResources(config);
    if (resources.length > 0) {
      files.push({
        path: "src/resources.ts",
        content: generateResourcesFile(resources),
        language: "typescript",
        description: "Recursos MCP - contexto y criterios para el AI",
      });
    }
  ...
  private generateMainServer(
    ...
    // Check if we have resources to register
    const resources = generateResources(config);
    const hasResources = resources.length > 0;
    const resourceImport = hasResources
      ? `import { registerResources } from "./resources.js";`
      : "";
    const resourceRegistration = hasResources
      ? `\n// Register resources (context for AI)\nregisterResources(server);`
      : "";
    const resourceCapability = hasResources ? `\n    resources: {},` : "";
 ...
 * Transport: stdio
 * Access Strategy: ${config.accessStrategy || "code_input"}
